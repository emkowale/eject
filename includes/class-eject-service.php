...
class Eject_Service {
    ...
    public static function delete_po(int $po_id): array {
        ...
        foreach ($order_ids as $oid) {
            ...
            if ($ordered) {
                $order->add_order_note(sprintf('Eject: ordered PO %s deleted', $po_number ?: $po_id), false, true);
            }
            $order->save();
        }
    ...
    public static function mark_po_ordered(int $po_id, bool $has_unordered = false, array $keep_items = []): array {
        ...
        foreach ($order_ids as $oid) {
            $order = wc_get_order($oid);
            if ($order) {
                $order->add_order_note(sprintf('Eject: PO %s ordered%s', $po_number, $vendor ? (' for '.$vendor) : ''), false, true);
                $order->save();
            }
        ...
        foreach ($pos as $pid) {
            ...
            if ($all_ordered) {
                ...
                if ($order->get_status() !== 'on-order') {
                    $order->update_status('on-order', 'Eject: all vendor POs ordered.');
                }
            } else {
        ...
        foreach ($prev_orders as $oid) {
            ...
            if (!$still_used) {
                ...
                if ($order->get_status() !== 'on-hold') {
                    if ($ordered) {
                        $order->update_status('on-hold', 'Eject: items removed from PO '.$po_number);
                    } else {
                        $order->set_status('on-hold');
                    }
                }
                if ($ordered) {
                    $order->add_order_note(sprintf('Eject: items removed from ordered PO %s', $po_number), false, true);
                }
                $order->save();
            } else {
                ...
                if ($order->get_status() !== 'on-hold') {
                    if ($ordered) {
                        $order->update_status('on-hold', 'Eject: some items removed from PO '.$po_number);
                    } else {
                        $order->set_status('on-hold');
                    }
                }
                if ($ordered) {
                    $order->add_order_note(sprintf('Eject: some items removed from ordered PO %s', $po_number), false, true);
                }
                $order->save();
            }
        }
